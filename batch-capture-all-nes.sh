#!/usr/bin/env zsh
#
# Batch capture all NES lesson screenshots using the working nes-simple-screenshot.sh script
#

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
CAPTURE_SCRIPT="$SCRIPT_DIR/nes-simple-screenshot.sh"
...
echo ""
echo "${BLUE}╔════════════════════════════════════════════╗${NC}"
echo "${BLUE}║  NES Screenshot Batch Capture              ║${NC}"
echo "${BLUE}║  Using nes-simple-screenshot.sh            ║${NC}"
echo "${BLUE}╚════════════════════════════════════════════╝${NC}"
echo ""
...
for lesson_dir in "$CODE_SAMPLES"/lesson-*(N); do
    ...
    # Capture screenshot
    echo "${YELLOW}📸 Capturing $lesson_num...${NC}"

    if "$CAPTURE_SCRIPT" "$nes_file" "$output_file" 300 2>&1 | tail -1; then
        SUCCESS=$((SUCCESS + 1))
    else
